...
# varImp(median_mdl)
# Relating to the model, the most important variables were 'ShareWoman', 'P25th,' 'Major_category_Sciences,' and 'P75th' all with an overall respective importance of 100% when predicting the salary (median) variable. Similar to those variables, 'Major_category_STEM' is also an important variables that relate to the prediction of the median classifier variable, with an overall importance of approximately 95%. These are viable results, given that one's percentile of earnings and the category for which their majory is categorized, should all have a high influence of one's respective yearly earnings. 
```
